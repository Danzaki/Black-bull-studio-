    ...
    <div className="min-h-screen bg-[#090A0F] text-slate-100 flex flex-col font-sans max-w-md mx-auto relative pb-20 select-none">
      ...
      <header className="p-4 border-b border-slate-800/80 bg-[#0D0E15] sticky top-0 z-20">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-xs text-slate-400 block font-medium">Terminal Wallet</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="px-3 py-1.5 bg-emerald-500 text-black font-extrabold text-xs rounded-full flex items-center gap-1 shadow-lg shadow-emerald-500/20">
              <Zap className="w-3.5 h-3.5 fill-black" /> Instant Snipe
            </button>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search CA / Token / Wallet Address..."
            className="w-full bg-[#13151F] border border-slate-800/90 rounded-xl pl-9 pr-4 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 font-mono"
          />
        </div>
      </header>
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-950/60 via-slate-900 to-slate-900 border border-emerald-500/30 relative overflow-hidden">
          <div className="relative z-10">
            <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest bg-emerald-950/80 px-2 py-0.5 rounded border border-emerald-800/80">
              Black Bull Terminal V2
            </span>
            <h2 className="text-lg font-black text-white mt-1">Institutional Speed Trading</h2>
            <p className="text-xs text-slate-400 mt-0.5">Real-time GMGN & Pump.fun Alpha Stream</p>
          </div>
        </div>
      </div>

      {/* 4. TABS HEADER */}
      <div className="px-4 flex items-center justify-between border-b border-slate-800/80 text-xs font-bold">
        <button
          onClick={() => setActiveTab('hot')}
          className={`pb-2.5 transition relative ${
            activeTab === 'hot' ? 'text-emerald-400 border-b-2 border-emerald-500' : 'text-slate-500'
          }`}
        >
          🔥 Hot Searches
        </button>
        <button
          ...
          className={`pb-2.5 transition relative ${
            activeTab === 'trenches' ? 'text-emerald-400 border-b-2 border-emerald-500' : 'text-slate-500'
          }`}
        >
          ⚡ Trenches (Pump)
        </button>
        <button
          ...
          className={`pb-2.5 transition relative ${
            activeTab === 'callouts' ? 'text-emerald-400 border-b-2 border-emerald-500' : 'text-slate-500'
          }`}
        >
          💬 Callouts & Feed
        </button>
      ...
      <div className="p-4 space-y-3">
        ...
        {activeTab === 'trenches' && (
          <div className="space-y-2">
            {[
              { name: 'Cat Clan', symbol: '$CLAN', mcap: '$6.55M', change: '+118.42%', isUp: true },
              { name: 'Cash Cat', symbol: '$CASH', mcap: '$49.54M', change: '-3.71%', isUp: false },
              { name: 'Cate AI', symbol: '$CATE', mcap: '$14.00M', change: '-9.76%', isUp: false },
            ].map((item, idx) => (
              <div key={idx} className="p-3 bg-[#0D0E15] border border-slate-800/80 rounded-xl flex items-center justify-between">
                <div>
                  <span className="font-bold text-sm text-white block">{item.name}</span>
                  <span className="text-xs text-slate-400 font-mono">{item.symbol} • {item.mcap}</span>
                </div>
                <button className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold ${item.isUp ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'bg-rose-500/10 text-rose-400 border border-rose-500/30'}`}>
                  {item.change}
                </button>
              </div>
            ))}
